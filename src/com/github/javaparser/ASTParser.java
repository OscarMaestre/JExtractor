    ...
    private Expression generateLambda(Expression ret, Statement lambdaBody) {
        if (ret instanceof EnclosedExpr) {
            ...
            if (inner != null && inner instanceof NameExpr) {
                VariableDeclaratorId id = new VariableDeclaratorId(inner.getRange(), ((NameExpr) inner).getName(), emptyList());
                NodeList<Parameter> params = add(emptyList(), new Parameter(ret.getRange(), EnumSet.noneOf(Modifier.class), emptyList(), new UnknownType(), emptyList(), false, id));
                ret = new LambdaExpr(range(ret.getBegin(), lambdaBody.getEnd()), params, lambdaBody, true);
            } else {
                ret = new LambdaExpr(range(ret.getBegin(), lambdaBody.getEnd()), emptyList(), lambdaBody, true);
            }
        } else if (ret instanceof NameExpr) {
            VariableDeclaratorId id = new VariableDeclaratorId(ret.getRange(), ((NameExpr) ret).getName(), emptyList());
            NodeList<Parameter> params = add(emptyList(), new Parameter(ret.getRange(), EnumSet.noneOf(Modifier.class), emptyList(), new UnknownType(), emptyList(), false, id));
...
isAsterisk = true;
      ...
      jj_consume_token(SEMICOLON);
ret = ImportDeclaration.create(range(begin, tokenEnd()), name, isStatic, isAsterisk);
      break;
      }
    case SEMICOLON:{
      jj_consume_token(SEMICOLON);
...
return ret;
  ...
  VariableDeclaratorId id;
    jj_consume_token(COMMA);
    id = VariableDeclaratorId();
ret = add(ret, new Parameter(range(id.getBegin(), id.getEnd()), EnumSet.noneOf(Modifier.class), emptyList(), new UnknownType(), emptyList(), false, id));
    label_19:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
      case COMMA:{
...
if (hasArgs) {
                                ret = new MethodCallExpr(range(ret.getBegin(), pos(token.endLine, token.endColumn)), ret, typeArgs.list, name, args);
                        } else {
                                ret = new FieldAccessExpr(range(ret.getBegin(), pos(token.endLine, token.endColumn)), ret, emptyList(), name);
                        }
        break;
        }
      case DOUBLECOLON:{
...
if(!isLambda) { ret = new EnclosedExpr(range(begin, tokenEnd()), ret);}
                                  ...
                                  if(ret != null){
                                          ...
                                          {
                                            id = new VariableDeclaratorId(range(ret.getBegin(), ret.getEnd()), ((NameExpr)ret).getName(), emptyList());
                                            p = new Parameter(range(ret.getBegin(), ret.getEnd()), EnumSet.noneOf(Modifier.class), emptyList(), new UnknownType(), emptyList(), false, id);
                                          }
...
if (hasArgs) {
                                ret = new MethodCallExpr(range(scope.getBegin(), tokenEnd()), scope, typeArgs.list, name, args);
                        } else {
                                ret =  new FieldAccessExpr(range(scope.getBegin(), tokenEnd()), scope, typeArgs.list, name);
                        }
        } else {
          jj_consume_token(-1);
          throw new ParseException();
    ...
    if (jj_2_36(2)) {
      ...
      case LPAREN:{
        ...
        case DOUBLECOLON:{
          ...
          case INCR:{
            jj_consume_token(INCR);
expr = new UnaryExpr(range(expr.getBegin(), tokenEnd()), expr, UnaryExpr.Operator.postIncrement);
            break;
            }
          case DECR:{
            jj_consume_token(DECR);
expr = new UnaryExpr(range(expr.getBegin(), tokenEnd()), expr, UnaryExpr.Operator.postDecrement);
            break;
            }
          case ASSIGN:
          case PLUSASSIGN:
...
if (exceptionTypes.size() > 1) {
                                ...
                                Parameter catchType = new Parameter(Range.UNKNOWN, exceptModifier.modifiers, exceptModifier.annotations, type, emptyList(), false, exceptId);
